...
async def service_update_meeting_location_edgedb(
    ...
    return await update_meeting_location(edgedb_client, url_code=meeting_url_code, location=location)


async def service_update_meeting_title_edgedb(meeting_url_code: str, title: str) -> UpdateMeetingTitleResult | None:
    return await update_meeting_title(edgedb_client, url_code=meeting_url_code, title=title)
